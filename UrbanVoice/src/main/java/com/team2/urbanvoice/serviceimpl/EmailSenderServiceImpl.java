...
import com.team2.urbanvoice.repositories.EmailOtpRepository;
...
public class EmailSenderServiceImpl implements EmailSenderService {
    ...
    @Autowired
    public EmailSenderServiceImpl(@Value("${OTPTIME}") int otpTime,
                                  @Value("${EMAIL_BODY}") String emailBody,
                                  @Value("${SECURITY_MESSAGE}") String securityMessage,
                                  @Value("${REGARDS}") String regards,
    ...
    public String getotp(String email) {
        ...
        String body = emailBody + otp + "\n" + securityMessage + "\n" + regards;
        try {
            sendSimpleEmail(email, emailSubject, body);
    ...
    private void sendSimpleEmail(String toEmail, String subject, String body) {
        log.debug("Sending Email with OTP to: {}", toEmail);
	...
	@Override
	public Boolean validate(String email, String otp) throws UrbanVoiceException {
		...
		if (emailOtp != null) {
			...
			throw new UrbanVoiceException("Otp is expired");
		}
	}
